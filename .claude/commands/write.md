...
- **File Location**: `content/drafts/[topic-slug]-[YYYY-MM-DD].md`
...
Example: `content/drafts/content-marketing-strategies-2025-10-15.md`
...
### Example Workflow
```
1. Write article → Save to content/drafts/article-name-2025-10-31.md
...
- **File**: `content/drafts/content-analysis-[topic-slug]-[YYYY-MM-DD].md`
...
- **File**: `content/drafts/keyword-analysis-[topic-slug]-[YYYY-MM-DD].md`
...
- **Score ≥ 70**: Save to `content/drafts/` and proceed to optimization agents
- **Score < 70 after 2 iterations**: Save to `content/review-required/` with a `_REVIEW_NOTES.md` file containing the scoring details and remaining issues

### Review-Required Folder
Articles that fail quality threshold after 2 revision attempts go to `content/review-required/`:
```
...
## Empire-Specific Requirements

### GEO Optimization (All Sites)
Every article MUST include a **40–60 word direct answer block** beneath each H2 heading. This is for AI/LLM citation optimization. Format:

```
## [H2 Heading]

[40–60 word direct answer that could stand alone as a citation. No fluff. Lead with the answer, not context.]
...
After `/optimize` and `/scrub`, move to: `content/published/[topic-slug].md`
This file is then copied into `src/app/blog/[slug]/content.md` (or site-equivalent) for Next.js SSR integration.
